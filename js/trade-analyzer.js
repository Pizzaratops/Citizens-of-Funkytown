...
//  BEWERTUNG (Redraft)
// ============================================================
//  Grundlage ist der 2026/27 Projections Rang (getProjRank bzw.
//  _getProjRankMap in js/navigation.js, taeglich aus LIVE_PROJECTIONS).
//  Wert = TRADE_VALUE_TABLE[Rang], ohne Altersanpassung: in einer
//  Redraft-Liga zaehlt nur diese Saison. Die fruehere Dynasty-/Win-Now-
//  Umschaltung und Draft Picks als Handelsgut wurden am 2026-09-23
//  entfernt.
function tradePlayerValue(rank) {
  if (rank == null) return 0;
  const v = TRADE_VALUE_TABLE[rank];
  return v === undefined ? 0 : v; // Out-of-range Raenge (tiefer Tail) -> 0
}

// Aggregate trade value with diminishing returns per additional player.
function tradeSideValue(players) {
  if (!players.length) return 0;
  const withValues = players.map(p => tradePlayerValue(p.rank)).sort((a, b) => b - a);
  let total = 0;
...
// Spielerpool = alle Spieler mit Projection, nach Rang sortiert. Der Rang
// ist eindeutig (1..N) und dient zugleich als Schluessel fuer die Auswahl.
// Team/Position aus dem Roster, sonst aus PLAYER_DB.
function buildTradePlayerPool() {
  const ownerMap = buildFantasyOwnerMap();
  const rosterMeta = new Map();
  Object.values(ROSTERS).flat().forEach(p => rosterMeta.set(normalizeName(p.name).toLowerCase(), p));
  const names = typeof LIVE_PROJECTIONS !== 'undefined' ? Object.keys(LIVE_PROJECTIONS) : [];
  return names
    .map(name => ({ name, rank: getProjRank(name) }))
    .filter(p => p.rank !== null)
    .sort((a, b) => a.rank - b.rank)
    .map(({ name, rank }) => {
      const key = normalizeName(name).toLowerCase();
      const ro = rosterMeta.get(key);
      const db = getPlayerDbEntry(name);
      const ownerId = ownerMap[normalizeName(name)] || null;
      return {
        isPick: false, rank, name,
        nba: (ro && ro.team) || (db && db[1]) || 'FA',
        pos: (ro && ro.pos) || (db && db[2]) || '',
        dob: db ? db[3] : null,
        owner: ownerId ? teamMap[ownerId] : null, ownerId,
      };
    });
}
...
function initTradeDropdowns() {
  const nbaAbbrs = [...new Set(buildTradePlayerPool().map(p => p.nba))].filter(a => a && a !== 'FA').sort();
  ...
  ['A','B'].forEach(side => {
    ...
    document.getElementById('tradeTTFilter'+side).innerHTML = `<option value="">🕺 Liga-Team</option>${ttOpts}`;
  });
}
...
function renderTradeList(side) {
    ...
    return `<div class="trade-player-item ${isSel ? 'selected' : ''}" onclick="toggleTradePlayer('${side}', ${p.rank})">
      ...
      <div class="tp-name">${p.name}
        ...
        <div style="display:flex;gap:4px;margin-top:4px;flex-wrap:wrap;">
          ${rankBadgeHtml('Proj', p.rank, rankStyle(p.rank))}
        </div>
      </div>
  ...
  }).join('') || '<div class="trade-no-players">Keine Spieler gefunden.</div>';
}
...
function renderTradeResult() {
  ...
  if (diff < 5) {
    ...
    subtext = 'Both sides receive roughly equal value for this season.';
  } else if (diff < 12) {
    ...
    cls = 'lopsided';
    subtext = `Side ${aWins ? 'A' : 'B'} gets significantly more value for this season.`;
  }

  function breakdownSide(players, label, sideVal) {
    // Sort by value for display
const sorted = [...players].sort((a, b) => tradePlayerValue(b.rank) - tradePlayerValue(a.rank));   const rows = sorted.map((p, i) => {
      const rawVal       = tradePlayerValue(p.rank);
      const effectiveVal = Math.round(rawVal * Math.pow(0.80, sorted.indexOf(p)));
      const isDiscounted = i > 0;
      const isLight      = document.body.classList.contains('light');

      const rankSection = `<div style="display:flex;gap:4px;margin-top:5px;flex-wrap:wrap;">
          ${rankBadgeHtml('Proj', p.rank, rankStyle(p.rank))}
        </div>`;
      ...
      return `<div class="trade-breakdown-player">
        <div style="flex:1;">
          ...
          <div class="tbp-detail">${p.nba} · ${ownerLabel}</div>
          ${rankSection}
        </div>
  ...
  if (el) el.innerHTML = `<strong style="color:var(--text);">So wird bewertet:</strong> Jeder Spieler bekommt den Wert seines aktuellen 2026/27 Projections Rangs aus der Wertetabelle (Rang 1 = 1.590, Rang 50 = 325, ab Rang 528 = 0). Keine Altersanpassung, in einer Redraft-Liga zählt nur diese Saison. Jeder weitere Spieler im Paket zählt <strong>20% weniger</strong> als der vorige (abnehmender Grenznutzen).`;
}
...
function openTradeShareModal() {
  ...
  if (!selA.length || !selB.length) {
    ...
    hint.textContent = 'Wähle Spieler auf beiden Seiten aus.';
    document.body.appendChild(hint);
  ...
  function pillRow(items, label, val) {
    const rows = [...items]
      .sort((a, b) => tradePlayerValue(b.rank) - tradePlayerValue(a.rank))
      .map((p, i) => {
        const line   = p.name;
        const detail = `${p.nba || ''}${p.owner ? ' · ' + p.owner.name : ''}`;
        const value  = tradePlayerValue(p.rank);
  ...
  const cardHtml = `<div id="shareCardInner" style="
    ...
    <div style="text-align:center;margin-bottom:14px;">
      <div style="font-size:10px;font-weight:700;letter-spacing:2px;color:${th.muted};text-transform:uppercase;margin-bottom:5px;">🕺 Citizens of Funkytown · Trade Analyzer</div>
      <div style="font-family:'Playfair Display',Georgia,serif;font-size:22px;font-weight:800;color:${th.accentA};">${verdict}</div>
      <div style="font-size:11px;color:${th.muted};margin-top:4px;">Bewertung nach 2026/27 Projections</div>
    </div>
    ...
    <div style="text-align:center;margin-top:14px;font-size:9px;color:${th.muted};letter-spacing:1px;text-transform:uppercase;">Citizens of Funkytown</div>
  </div>`;
...
function buildTradeShareText(selA, selB, valA, valB, verdict) {
  ...
  return [
    '🕺 Citizens of Funkytown · Trade Analyzer',
...
async function downloadTradeShare() {
  ...
  try {
    ...
    link.download = `funkytown-trade-${stamp}.png`;
    link.click();
